...
def parse_python_backend(args: list[str]) -> tuple[RobocopyConfig, bool]:
    ...
    copy_group.add_argument("-s", "--subdirs", action="store_true", help="Copy subdirectories")
    copy_group.add_argument("-e", "--empty-subdirs", action="store_true", help="Copy empty subdirectories")
    copy_group.add_argument("-z", "--restartable", action="store_true", help="Restartable mode")
    copy_group.add_argument("-b", "--backup-mode", action="store_true", help="Backup mode")
    copy_group.add_argument("-mt", "--multi-threaded", type=int, default=8, help="Multithreading count")
    copy_group.add_argument("-fft", "--fat-file-times", action="store_true", default=True, help="FAT file times")
    copy_group.add_argument("--no-fat-file-times", action="store_false", dest="fat_file_times")
    copy_group.add_argument("-copy", "--copy-flags", type=str, default="DAT", help="Copy flags (default: DAT)")
    copy_group.add_argument("-dcopy", "--dir-copy-flags", type=str, default="DA", help="Dir copy flags (default: DA)")
    copy_group.add_argument("-purge", "--purge", action="store_true", help="Purge destination files")
    copy_group.add_argument("-mir", "--mirror", action="store_true", help="Mirror mode")

    # Selection options
    sel_group = parser.add_argument_group("Selection Options")
    sel_group.add_argument("-xo", "--exclude-older", action="store_true", default=True, help="Exclude older files")
    sel_group.add_argument("--no-exclude-older", action="store_false", dest="exclude_older")
    sel_group.add_argument("-xx", "--exclude-extra", action="store_true", help="Exclude extra files")
    sel_group.add_argument("-xf", "--exclude-files", nargs="*", default=[], help="Exclude files")
    sel_group.add_argument("-xd", "--exclude-dirs", nargs="*", default=[], help="Exclude dirs")
    sel_group.add_argument("-a", "--include-archive-only", action="store_true", help="Include archive only")
    sel_group.add_argument("-m", "--reset-archive", action="store_true", help="Reset archive attribute")

    # Logging options
    log_group = parser.add_argument_group("Logging Options")
    log_group.add_argument("-v", "--verbose", action="store_true", help="Verbose logging")
    log_group.add_argument("-nfl", "--no-file-list", action="store_true", help="No file list")
    log_group.add_argument("-ndl", "--no-dir-list", action="store_true", default=True, help="No dir list")
    log_group.add_argument("--list-dirs", action="store_false", dest="no_dir_list")
    log_group.add_argument("-ts", "--show-timestamps", action="store_true", help="Show timestamps")
    log_group.add_argument("-fp", "--full-pathnames", action="store_true", help="Full pathnames")
    log_group.add_argument("-bytes", "--bytes-as-integers", action="store_true", default=True, help="Bytes as integers")
    log_group.add_argument("-njh", "--no-job-header", action="store_true", help="No job header")
    log_group.add_argument("-njs", "--no-job-summary", action="store_true", help="No job summary")
    log_group.add_argument("-tee", "--tee", action="store_true", help="Tee output")

    # Retries
    parser.add_argument("-r", "--retry-count", type=int, default=3, help="Retry count")
    parser.add_argument("-w", "--retry-wait", type=int, default=3, help="Retry wait")
